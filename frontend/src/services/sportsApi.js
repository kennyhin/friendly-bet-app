...
export const SPORTS = [
  ...
  { key: 'ncaab', path: 'basketball/mens-college-basketball', name: 'NCAA',  emoji: '🎓' },
  { key: 'mls',   path: 'soccer/usa.1',                       name: 'MLS',   emoji: '⚽' },
]

export const PROP_STATS = {
  nfl:   ['Passing Yards', 'Passing TDs', 'Rushing Yards', 'Receiving Yards', 'Receptions', 'Interceptions'],
  nba:   ['Points', 'Rebounds', 'Assists', 'Steals', 'Blocks', '3-Pointers Made', 'Pts+Reb+Ast'],
  mlb:   ['Strikeouts', 'Hits', 'RBIs', 'Home Runs', 'Walks', 'Total Bases'],
  nhl:   ['Goals', 'Assists', 'Points', 'Shots on Goal', 'Saves'],
  ncaaf: ['Passing Yards', 'Rushing Yards', 'Receiving Yards', 'Touchdowns'],
  ncaab: ['Points', 'Rebounds', 'Assists', '3-Pointers Made'],
  mls:   ['Goals', 'Assists', 'Shots on Goal', 'Saves'],
}
...
export function fmtSpread(spread, forHome = false) {
  ...
  // ESPN: spread is from the away team's perspective (negative = away favored)
...
// Build the bet description strings from a game + betType + side
export function buildBetPicks(game, betType, side, propPlayer, propStat, propLine) {
  ...
  if (betType === 'moneyline') {
    ...
    return {
      event:        `${away} @ ${home}`,
      creatorPick:  `${myTeam} ML (${myML})`,
      opponentPick: `${oppTeam} ML (${oppML})`,
    }
  }
  ...
  if (betType === 'spread') {
    ...
    return {
      event:        `${away} @ ${home}`,
      creatorPick:  `${myTeam} ${mySpread} (${myOdds})`,
      opponentPick: `${oppTeam} ${oppSpread} (${oppOdds})`,
    }
  }
  ...
  if (betType === 'over_under') {
    ...
    return {
      event:        `${away} @ ${home}`,
      creatorPick:  `${side === 'over' ? 'Over' : 'Under'} ${total} pts`,
      opponentPick: `${side === 'over' ? 'Under' : 'Over'} ${total} pts`,
    }
  }
